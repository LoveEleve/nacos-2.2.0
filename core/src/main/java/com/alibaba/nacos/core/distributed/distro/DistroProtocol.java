...
public class DistroProtocol {
    ...
    private void startDistroTask() {
        // ★ 验证探针C：startDistroTask - 判断 standalone 模式
        ...
        if (EnvUtil.getStandaloneMode()) {
            isInitialized = true;
            // ★ 验证探针D：standalone 模式直接跳过加载和验证任务
        ...
        System.out.println("[PROBE][DistroProtocol.startDistroTask] 集群模式: 启动VerifyTask和LoadTask"
                + ", verifyIntervalMs=" + DistroConfig.getInstance().getVerifyIntervalMillis()
                + ", loadTimeoutMs=" + DistroConfig.getInstance().getLoadDataTimeoutMillis());
        startVerifyTask();
        startLoadTask();
    }
    ...
    public void sync(DistroKey distroKey, DataOperation action, long delay) {
        // ★ 验证探针E：sync - 同步目标节点数量
        System.out.println("[PROBE][DistroProtocol.sync] resourceKey=" + distroKey.getResourceKey()
                + ", action=" + action
                + ", delay=" + delay + "ms"
                + ", targetNodeCount=" + memberManager.allMembersWithoutSelf().size());
    ...
    public void syncToTarget(DistroKey distroKey, DataOperation action, String targetServer, long delay) {
        ...
        // ★ 验证探针F：syncToTarget - 延迟任务入队
        System.out.println("[PROBE][DistroProtocol.syncToTarget] 延迟任务入队"
                + ", resourceKey=" + distroKey.getResourceKey()
                + ", resourceType=" + distroKey.getResourceType()
