...
public class DistroDelayTask extends AbstractDelayTask {
    ...
    public void merge(AbstractDelayTask task) {
        ...
        // ★ 验证探针G：merge - 延迟任务合并逻辑
        String oldAction = oldTask.getAction().toString();
        String newAction = action.toString();
        boolean willMerge = !action.equals(oldTask.getAction()) && createTime < oldTask.getCreateTime();
        System.out.println("[PROBE][DistroDelayTask.merge] 合并任务"
                + ", resourceKey=" + distroKey.getResourceKey()
                + ", currentAction=" + newAction
                + ", incomingAction=" + oldAction
                + ", currentCreateTime=" + createTime
                + ", incomingCreateTime=" + oldTask.getCreateTime()
                + ", willUpdateAction=" + willMerge);
        if (!action.equals(oldTask.getAction()) && createTime < oldTask.getCreateTime()) {
            ...
            System.out.println("[PROBE][DistroDelayTask.merge] action已更新: " + oldAction + " -> " + action);
        }
