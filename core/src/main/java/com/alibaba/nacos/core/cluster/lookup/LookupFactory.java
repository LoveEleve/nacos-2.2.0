...
public final class LookupFactory {
    ...
    public static MemberLookup createLookUp(ServerMemberManager memberManager) throws NacosException {
        System.out.println("[PROBE][LookupFactory.createLookUp] standaloneMode=" + EnvUtil.getStandaloneMode());
        if (!EnvUtil.getStandaloneMode()) {
            String lookupType = EnvUtil.getProperty(LOOKUP_MODE_TYPE);
            LookupType type = chooseLookup(lookupType);
            LOOK_UP = find(type);
            currentLookupType = type;
            System.out.println("[PROBE][LookupFactory.createLookUp] 集群模式: lookupTypeProperty=" + lookupType + ", 选择类型=" + type + ", 实现类=" + LOOK_UP.getClass().getSimpleName());
        } else {
            LOOK_UP = new StandaloneMemberLookup();
            System.out.println("[PROBE][LookupFactory.createLookUp] 单机模式: 直接使用StandaloneMemberLookup");
        }
    ...
    private static LookupType chooseLookup(String lookupType) {
        if (StringUtils.isNotBlank(lookupType)) {
            ...
            if (Objects.nonNull(type)) {
                System.out.println("[PROBE][LookupFactory.chooseLookup] 显式配置lookupType=" + lookupType + ", 选择=" + type);
                return type;
            }
        }
        File file = new File(EnvUtil.getClusterConfFilePath());
        boolean fileExists = file.exists();
        boolean memberListNotBlank = StringUtils.isNotBlank(EnvUtil.getMemberList());
        System.out.println("[PROBE][LookupFactory.chooseLookup] cluster.conf路径=" + file.getAbsolutePath() + ", 文件存在=" + fileExists + ", memberList非空=" + memberListNotBlank);
        if (fileExists || memberListNotBlank) {
            System.out.println("[PROBE][LookupFactory.chooseLookup] 选择FILE_CONFIG模式");
            return LookupType.FILE_CONFIG;
        }
        System.out.println("[PROBE][LookupFactory.chooseLookup] 选择ADDRESS_SERVER模式");
