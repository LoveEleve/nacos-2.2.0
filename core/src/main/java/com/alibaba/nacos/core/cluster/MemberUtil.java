...
public class MemberUtil {
    ...
    public static void onSuccess(final ServerMemberManager manager, final Member member) {
        ...
        if (!Objects.equals(old, member.getState())) {
            System.out.println("[PROBE][MemberUtil.onSuccess] 节点状态恢复: " + old + " -> UP, address=" + member.getAddress());
    ...
    public static void onFail(final ServerMemberManager manager, final Member member, Throwable ex) {
        ...
        System.out.println("[PROBE][MemberUtil.onFail] 节点访问失败: " + old + " -> " + member.getState() + ", address=" + member.getAddress() + ", failCnt=" + member.getFailAccessCnt() + "/" + maxFailAccessCnt + ", ex=" + (ex.getMessage() != null ? ex.getMessage().substring(0, Math.min(50, ex.getMessage().length())) : "null"));
        if (!Objects.equals(old, member.getState())) {
            manager.notifyMemberChange(member);
        }
    }
